...
class Graph
{
  ...
  public:
    ...
    /** Small class for exceptions. Derives from std::exception implementing the
       ...
       MMCFGException class are thought to be of the "fatal" type, i.e.,
       problems for which no solutions exists apart from aborting the program.
       Other kinds of exceptions can be properly handled by defining derived
       classes with more information. */
    ...
    /**< Constructor of the Class: builds an instance of MMCF reading the data
       ...
       - 's'  (default) for single-file "Canadian" format; this format is
       actually for Fixed-Charge MMCF, hence there is a Fixed Charge Cost
       information attached to each arc. Thus, m extra variables are defined,
       their cost is set to this Fixed Charge Cost, their upper and lower bounds
       are set set to 0 and 1, respectively, and their type is set to integer;

       - 'c'  for single-file PPRN format: actually, the format read by this
            constructor is a bit more general than the standard definition,
       since it is permitted for an arc not to have single-commodity capacity
       (-1);

       - 'p'  for a (PSP) formulation  \
       - 'o'  for a (OSP) formulation  | for Jones-Lustig (JL) standard
       four-files
       - 'd'  for a (ODP) formulation  | format
       - 'u'                           /

       - 'm'  for the four-files mnetgen format, an extension of the original
       file format produced by mnetgen (see mnetgen.C), partially compatible
       with the JL format

       All the four-files formats read the instance from the 4 files `FN'.nod
       (the instance size), `FN'.sup (node supply information - but see 'u'),
       `FN'.arc (arc information) and `FN'.mut (mutual capacity constraints
       information). The 'o' and 'u' formats are essentially the same, but with
       'u' the node supply informations are searched in a `FN'.od.
    ...
    /**< Constructor of the class: builds an instance of MMCF reading the data
       ...
       - Cost    is the (K x m)-matrix of the arc costs: Cost[ k ][ i ] is the
                 ...
                 graphs to be different for each commodity: this is stronger
       then setting U[ k ][ i ] == 0 [see PreProcess() below]; Cost can be 0,
                 meaning that all costs are 0, and likewise each Cost[ k ] can
                 be 0, meaning that all the costs for commodity k are 0;

       Arc i for commodity k is non-existent if Cost[ k ][ i ] ==
       Inf<CNumber>(), and has no single-commodity capacity if U[ k ][ i ] ==
       Inf<FNumber>(). Arc i has no mutual capacity if CapTot[ i ] ==
       Inf<FNumber>(). Node i for commodity k is non-existent if Def[ k ][ i ]
       == Inf<FNumber>(). All arcs entering/leaving a non-existent node for
       commodity k should be non-existent; this is guaranteed by PreProcess()
       [see below].

       This constructor builds a problem with no extra variables and
       constraints, and with all the (flow) variables continuous. This can be
       changed later with SetIntVar(), SetExtraVar() and SetExtraConstr() [see
       below].

       Memory saving trick: sometimes, all arcs have the same capacity or cost,
       independently from the commodity. It is clearly possible to construct
       only one m-vector containing the capacities/costs, and copy its pointer k
       times in Cap[]/Cost[]. The same holds for deficits. Note that,
       internally, the Graph object will initially allocate all memory anyway
       and copy K times the information to separate vectors; however, the
       redundancy will be eliminated in PreProcess() [see below], if it
       "survives" (preprocessing can alter the values of the data, thus making
       initially identical vectors different). */

    /*--------------------------------------------------------------------------*/

    inline Graph(void){};

    /**< "base" constructor of the class, that in fact does nothing: is tought
       as a mean for derived class to provide their own initialization of the
       data structures, in case they are incompatible with the ones given in the
    ...
    /**< NrExtraConst() returns the total number of "extra" constraints.
       ...
       representation the "extra" constraints i, i + 1, ... j as linear
       two-sided inequalities. The "names" (indices) of "extra" constraints go
       from 0 to NrExtraConst() - 1. It is illegal to call NrExtraNonZ() if
       there are no "extra" constraints, i.e., NrExtraConst() == 0. */
    ...
    /**< Returns the type of the k-th subproblem: it can be a generic Min Cost
       Flow problem or a Shortest Path Tree problem. The type is initially set
       to kMCF, and turned to kSPT (if it is the case) only if PreProcessing()
       or MakeSingleSourced() [see below] are invoked. */
    ...
    /**<The first form returns a read-only pointer to a m-vector containing the
       mutual capacities of the arcs; the second form returns the mutual
       capacity of arc j. */
    ...
    /**< The first form returns a read-only pointer to a m-vector containing the
       ...
       For k == NrComm() and k == NrComm() + 1, the lower and upper bounds on
       the "extra" (non-flow) variables are returned, respectively: it is
       illegal to call these methods with k > NrComm() if there are no extra
       variables, i.e. NrExtraVars() == 0. */
    ...
    /**< The first form returns a read-only pointer to a n-vector containing the
       node deficits relative to commodity k; the second form returns the
       deficit of node j relative to commodity k.

       For k == NrComm() and k == NrComm() + 1, the lower and upper ranges of
       the "extra" constraints are returned, respectively: it is illegal to call
    ...
    /**< The first form returns a read-only pointer to a m-vector containing the
       ending nodes of the arcs; the second form returns the ending node of arc
       j.
       */
    ...
    /**< Some of the variables of the MMCF-like problem may be constrained to be
       ...
       NIntVar( k ) == 0 or NIntVar( k ) == NrArcs(), then WIntVar( k ) returns
       0; none/all the variables for commodity k are integer-valued.
       ...
       the problem which are constrained to be integer-valued, counting both
       flow and extra variables. */

    /*--------------------------------------------------------------------------*/

    virtual void ExtraConstr(int* IBeg,
                             int* Indx,
                             double* Vals,
                             Index FrstC = 0,
                             Index LstC = Inf<Index>());
    ...
    /**< Writes in IBeg, Indx and Vals the description of those "extra" linear
       ...
       set of indices of variables with nonzero coefficient and the
       corresponding coefficients. The indices and coefficients corresponding to
       the i-th constraint that is returned, i = 0, ..., LstC - FrstC, are
       written in Indx and Vals, respectively, in the positions between IBeg[ i
       ...
       has the index k * m + j; the i-th "extra" variable (0 <= i <
       NrExtraVars()) has the index K * m + i (the representation is
       "commodity-wise", with the "extra" variables following). */
    ...
    /**< Decides if the graph G of the problem has to be considered as a
       directed (DG == true) or an undirected (DG == false) graph, setting the
       value that is returned by Directed( void ) [see above].
    ...
    /**< Updates the total upper capacities, either all of them or just the
       j-th, i.e. the one corresponding to the j-th arc. 0 means "all
       Inf<FNumber>()". */
    ...
    /**< Update the single-commodity capacities, either all of the commodity k
       or just the one relative to the j-th arc for commodity k. 0 means "all
       Inf<FNumber>()".

       For k == NrComm() and k == NrComm() + 1, the lower and upper bounds on
       the "extra" (non-flow) variables are changed, respectively: it is illegal
       to call these methods with k >= NrComm() if there are no extra variables,
       i.e. NrExtraVars() == 0. */
    ...
    /**< Update the node deficits, either all of the commodity k or just the one
       ...
       For k == NrComm() and k == NrComm() + 1, the lower and upper ranges of
       the "extra" constraints are changed, respectively: it is illegal to call
       these methods with k >= NrComm() if there are no extra constraints, i.e.
       NrExtraConst() == 0. */

    /*--------------------------------------------------------------------------*/

    virtual void SetIntVar(cIndex k,
                           bool IntVld = true,
                           cIndex_Set nms = 0,
                           cIndex strt = 0,
                           Index stp = Inf<Index>());
    ...
    /**< Gives the Graph object the information about which among the (flow and
       ...
       The original status of the variable when the object is constructed
       depends on the constructor used and on the arguments passed to it [see
       above]. */
    ...
    /**< Tells the Graph object that there are NXV "extra" (non-flow) variables
       in the problem.
       ...
       0, 0 and Inf<FNumber>() uniformly. Also; by default all extra variables
       are continuous, although this can be changed later with SetIntVar().
    ...
    /**< Gives to the Graph object a description of the "extra" constraints in
       the problem: NXC is the number of such constraints, and IStp, Indx and
       Vals must contain the description.

       The description is constraint-wise: each constraint is represented by the
       set of indices of variables with nonzero coefficient and the
       corresponding coefficients. The indices and coefficients corresponding to
       the i-th constraint, i = 0, ..., NXC - 1, are to be found in Indx and
       ...
       This is the very same format that is returned by ExtraConstr() [see
       above]. Actually, ExtraConstr() (at least, in the implementation of the
       base Graph class) will return exactly the pointers passed to this method,
       that will be retained inside the Graph object. That is, after the call
       the vectors become "property" of the Graph object and should not be
       changed. This method can be called more than once: at each call, the
       previous pointers (if any) are *deallocated* and substituted with the
       newly provided ones. Also, the pointers are deallocated in the destructor
       of Graph. A call to SetExtraConstr( 0 , ... ) deallocates all the memory
       reserved for extra constraints information.
    ...
    virtual void PreProcess(cFNumber IncUk = 0,
                            cFNumber DecUk = 0,
                            cFNumber IncUjk = 0,
                            cFNumber DecUjk = 0,
                            cFNumber ChgDfct = 0,
    ...
    /**< Performs various pre-processing of the data, trying to make the
       instance more easily solvable. The parameters to be given are the
       following:

       IncUk , DecUk   => (>= 0) upper bounds on the increase and decrease of
       the mutual capacities: may be Inf<FNumber>() if unknown;
       ...
       number, since it is used to generate "loose" but finite individual
       capacities for arcs that have none;
       ...
       For all k such that, after the pre-processing, the graph has only a
       source and no (existing) arcs have a "real" capacity, the type of the
       subproblem is set to kSPT: all other problem types (see ProblemType( k )
    ...
    /**< This method scans the graph and makes all the commodities
       single-sourced by adding, if necessary, one node to the graph and
       connecting it to all sources with arcs having individual capacities equal
       to the capacity of
       ...
       source, unless ToALL == true: in this case, the super-source is
       constructed anyway and it is linked to *all* nodes i with arcs
    ...
    virtual void OutMPSFile(const char* const FN,
                            bool FxdClmns = false,
                            const char* const PN = 0);

    /**< Outputs the current MMCF problem as a Linear Programming problem in the
       standard MPS format: if FxdClmns == true, then the fixed-column MPS
       format is used, else (the default) the "modern" MPS format is used, where
       fields have not a fixed column positions but are just blank-delimited.
       `FN' is taken as the [path]name of the output file, and `PN' (if
       provided) is taken as the name of the problem, to be written in the NAME
       field of the MPS file. */
  ...
  protected:
    ...
    /**< Called at the end of any constructor, does some initializations that
       are common to them all: it is "protected" for allowing derived classes
       that use the "void" constructor to call it. */
...
inline Graph::Index
Graph::NrExtraNonZ(cIndex FrstC, cIndex LstC)
{
    if (NXtrC)
...
inline Graph::MCFType
Graph::ProblemType(cIndex k)
{
    return (PT[k]);
}
...
inline Graph::FNumber
Graph::TotalCapacityJ(cIndex j)
{
    return (UTot[j]);
}
...
inline Graph::CNumber
Graph::CostKJ(cIndex k, cIndex j)
{
    return (C[k][j]);
}
...
inline Graph::FNumber
Graph::CapacityKJ(cIndex k, cIndex j)
{
    return (U[k][j]);
}
...
inline Graph::FNumber
Graph::DeficitKJ(cIndex k, cIndex j)
{
    return (B[k][j]);
}
...
inline Graph::Index
Graph::NIntVar(cIndex k)
{
    if (k <= NComm)
        return (NInt[k]);
    else {
        Index cnt = 0;
        for (Index i = 0; i <= NComm;)
            cnt += NInt[i++];
...
inline void
Graph::ProblemType(const Graph::MCFType NewType, cIndex k)
{
    PT[k] = NewType;
}
...
inline void
Graph::NamesStartFrom1(bool NSF1)
{
    if (NSF1)
        StrtNme = 1;
    else
