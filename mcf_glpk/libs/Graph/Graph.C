...
template<class T>
static inline void
VectAssign(T* const g, const T x, Graph::cIndex n)
{
...
static inline void
VectAssign(T1* g1, const T2* g2, Graph::Index n)
{
    // g1 := g2
...
template<class T>
inline void
VectSubtract(T* const g, const T x, Graph::cIndex n)
{
...
template<class T>
static inline bool
EqualVect(const T* g1, const T* g2, Graph::Index n)
{
...
Graph::Graph(const char* const FN, char FT)
{
    ...
    if ((FT == 'd') || (FT == 'o')) {
        ...
        // in (ODS) count the different O/D pairs - - - - - - - - -
        if (FT == 'd')
            while (inFile >> origin) {
                GOODN(origin);
    ...
    switch (FT) {
        ...
        {       //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ...
            for (Index k;
                 inFile >> k;) { // read node-related info - - - - - - - -
                GOODP(k);
        ...
        {       //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ...
            if (NXtrC) { // if there are "extra" constraints- - - - - - - - - -
                ...
                for (Index i = 0;
                     i < NXtrC;) { // read "extra" Uppr./Lwr. bounds
                    inFile >> EL[i];
                    inFile >> EU[i++];
                }
                ...
                for (Index j;
                     inFile >> j;) { // read extra constraints description:
                    Index k;         // j = arc name
            ...
            for (int origin; inFile >> origin;) {
                ...
                if (((origin == -1) && (dest == -1)) ||
                    ...
                    throw(MMCFGException(
                      "Graph::Graph( file ): exactly one p/d in PSP"));
        ...
        {       // while reading supplies, commodity "names" are assigned - - -
            ...
            for (int origin; inFile >> origin;) {
                ...
                if (comm != -1) {
                    // origin or destination node for the given pair ( product ,
                    // origin )
                    ...
                    // origin or destination node for all the commodities ( k ,
                    // origin ) for each product k
    ...
    if (FourFiles) {
        ...
        {              // it is dealt with separatedly, since it's simpler: the
            ...
            while (inFile >> who) {
                if ((who <= 0) || (who > NArcs))
                    throw(
                      MMCFGException("Graph::Graph( file ): invalid arc name"));
                who--;
            ...
            for (Index from; inFile >> from;) { // first, the usual data reading
                ...
                // now, the main part: from the triplet (origin, destination,
                // product) plus the file type (p, d, o) a list of applicable
                // commodities is constructed and put into TempIdx: then the arc
                // will be replicated over all the commodities of the list in
                // all the three fields, a "-1" takes the place of a wildcard
                ...
                switch (FT) {
                    ...
                    case ('o'): // tougher, 4 subcases- - - - - - - - - - - - -
                        ...
                        if (comm != -1) {
                            ...
                            if (origin !=
                                -1) { // a specific commodity ( prod , origin )
                                ...
                                *TempIdx = i;
                                ...
                                for (TmpCommCntr = 0; i < StartOfK[comm];)
                                   ...
                                   -1) // all commodities with a given origin
                    ...
                    default: // == 'd', the toughest: 8 subcases- - - - - - - -
                        ...
                        if (comm != -1) {
                            ...
                            if (dest != -1) {
                                if (origin != -1) { // a specific commodity
                                                    // (prod, origin, dest)
                                    while ((i < StartOfK[comm]) &&
                                           (Destins[i] != Index(dest)) &&
                                           (Origins[i] != Index(origin)))
                                        i++;
                                ...
                                if (origin != -1) { // all commodities with a
                                                    // given (prod, orig)
                                    for (TmpCommCntr = 0; i < StartOfK[comm];
                                         i++)
                            ...
                            if (dest != -1) {
                                if (origin != -1) { // all commodities with a
                                                    // given (orig, dest)
                                    for (Index i = TmpCommCntr = 0; i < NComm;
                                         i++)
                                        if ((Destins[i] == Index(dest)) &&
                                            (Origins[i] == Index(origin)))
                                            TempIdx[TmpCommCntr++] = i;
                ...
                for (Index i = TmpCommCntr; i--;) {
                    ...
                    if (ptr) { // if ptr != 0 it's easy
                        who =
                          ptr - 1; // ( ptr - 1 ) is already the correct name
                        ...
                        while ((k < unbndld) && ((Startn[NCnst + k] != from) ||
                                                 (Endn[NCnst + k] != to) ||
                                                 (C[comm][NCnst + k] < C_INF)))
                            k++;

                        // search for an arc (from, to) already defined among
                        // the unbundled ones and whose "instance" relative to
                        // commodity comm has not already been taken: this is
                        // not the only way of accomodating unbundled arcs, (in
                        // case of multiple instances of an unbundled arc (i,
                        // j)), but it is easy to see that all the resulting
                        // problems, however you distribute the instances to
                        // arcs, are equivalent
...
void
Graph::ExtraConstr(int* IBeg, int* Indx, double* Vals, Index FrstC, Index LstC)
{
    if (NXtrC) {
...
void
Graph::UpDtTotCapJ(cIndex j, cFNumber NewUj)
{
    UTot[j] = NewUj;
}
...
void
Graph::UpDtArcCstK(cIndex k, cCRow NewCk)
{
    if (NewCk)
...
void
Graph::UpdateArcCstKJ(cIndex k, cIndex j, cCNumber NewCkj)
{
    C[k][j] = NewCkj;
}
...
void
Graph::UpDtArcCapK(cIndex k, cFRow NewUk)
{
    if (NewUk)
...
void
Graph::UpDtArcCapKJ(cIndex k, cIndex j, cFNumber NewUkj)
{
    U[k][j] = NewUkj;
}
...
void
Graph::UpDtNdeDfctK(cIndex k, cFRow NewDk)
{
    if (NewDk)
...
void
Graph::UpDtNdeDfctKJ(cIndex k, cIndex j, cFNumber NewDkj)
{
    B[k][j] = NewDkj;
}
...
void
Graph::SetIntVar(cIndex k, bool IntVld, cIndex_Set nms, cIndex strt, Index stp)
{
...
void
Graph::SetExtraConstr(cIndex NXC, int* IBeg, int* Indx, double* Vals)
{
    delete[] IdxBeg;
    delete[] CoefIdx;
    delete[] CoefVal;
...
void
Graph::PreProcess(cFNumber IncUk,
                  cFNumber DecUk,
                  cFNumber IncUjk,
                  cFNumber DecUjk,
                  cFNumber ChgDfct,
                  cCNumber DecCsts)
{
    ...
    for (Index i = NCnst = 0; i < NArcs; i++) {
        ...
        // compute is an upper bound on the max quantity of flow (of any
        // commodity) on arc i: if capacities can increase indefinitely, the
        // only bound is the total quantity of flow in the graph
        ...
        // capacities of an arc are identical, the arc is marked as "inactive";
        // this is an arbitrary choice, since one could as well keep it and
        // eliminate all the individual capacities
        ...
        for (Index i = 0; i < NArcs; i++) {
            ...
            if (DecUjk < F_INF) {
                ...
                if (U[k][i] >= tmpv[k] + DecUjk) {
                    // the constraint is redundant because there will never be
                    // that much flow in the graph
                ...
                if ((IncUk < F_INF) &&
                    ...
                    // if mutual capacities cannot increase forever, some
                    // individual capacities may be declared redundant by the
                    // mutual capacity note that the mutual capacity of an arc
                    // can be used to declare that the individual capacity is
                    // redundant only if the arc is "active", as "inactive" arcs
                    // precisely mean that no mutual capacity constraint is
                    // imposed on them (i.e., the value of UTot[ i ] is not
                    // really meaningful and can be ignored)
...
void
Graph::MakeSingleSourced(bool ToAll)
{
...
void
Graph::OutMPSFile(const char* const FN, bool FxdClmns, const char* const PN)
{
    if (!DrctdPrb)
        throw(MMCFGException(
    ...
    { // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for (Index k = 0; k < NComm; k++) {
            ...
            for (Index i = 0; i < NArcs; i++) {
                if (C[k][i] < C_INF) {
                    of << " x" << k << "_" << i << "\tobj\t" << C[k][i] << "\tf"
                       << k << "_" << Startn[i] << "\t-1" << endl
                       << " x" << k << "_" << i << "\tf" << k << "_" << Endn[i]
                       << "\t1";
